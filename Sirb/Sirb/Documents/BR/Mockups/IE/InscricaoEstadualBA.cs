...
namespace Sirb.Documents.BR.Mockups.IE
{
	internal class InscricaoEstadualBA : InscricaoEstadualBase
	{
		...
		private int GetRandomLength(Random random) => random.Next(2) == 1 ? 9 : 8;

		private int CalculateBeforeLastWeight(int index, int length) => (length == 8 ? 7 : 8) - index;

		private int CalculateLastWeight(int index, int length) => (length == 8 ? 8 : 9) - index;

		private int GetModuloValue(int[] values, int length)
		{
			int[] validationDigits = new int[] { 6, 7, 9 };
