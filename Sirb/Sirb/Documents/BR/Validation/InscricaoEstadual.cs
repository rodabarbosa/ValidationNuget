...
namespace Sirb.Documents.BR.Validation
{
    ...
    public static class InscricaoEstadual
    {
        ...
        private static string StateMaskRN(string value)
        {
            ...
            return onlyNumbersValue.Length == 9 ? Regex.Replace(onlyNumbersValue, @"(\d{2})(\d{3})(\d{3})(\d{1})", "$1.$2.$3-$4") : Regex.Replace(onlyNumbersValue, @"(\d{2})(\d{1})(\d{3})(\d{3})(\d{1})", "$1.$2.$3.$4-$5");
        }
        ...
        private static string StateMaskSP(string value)
        {
            ...
            return value.StartsWith("P", StringComparison.OrdinalIgnoreCase) ? Regex.Replace(aux, @"(\d{8})(\d{1})(\d{3})", "P-$1.$2/$3") : Regex.Replace(aux, @"(\d{3})(\d{3})(\d{3})(\d{3})", "$1.$2.$3.$4");
        }
        ...
        private static bool ValidateAC(string value)
        {
            ...
            string dv = digit1 + digit2.ToString();
        ...
        private static bool ValidateAL(string value)
        {
            ...
            int sum = 0;
            for (int i = 0; i < 8; i++) sum += int.Parse(value[i].ToString()) * (9 - i);
        ...
        private static bool ValidateBA(string value)
        {
            ...
            for (int i = 0; i < length - 2; i++) sum += int.Parse(value[i].ToString()) * (weight - i);
        ...
        private static bool ValidateMG(string value)
        {
            ...
            for (int i = 0; i < 11; i++) sb.Append(i == 3 ? $"0{value[i]}" : value[i].ToString());
            ...
            for (int i = 0; i < sb.Length; i++)
            {
                ...
                for (int j = 0; j < length; j++) sum += int.Parse(strcalculatedValue[j].ToString());
            }

            int sumAux = sum;
            while (sumAux % 10 != 0) sumAux++;
        ...
        private static bool ValidateRN(string value)
        {
            ...
            int sum = 0;
            for (int i = 0; i < length; i++) sum += int.Parse(value[i].ToString()) * (weight - i);
        ...
        private static bool ValidateRR(string value)
        {
            ...
            int sum = 0;
            for (int i = 0; i < 8; i++) sum += int.Parse(value[i].ToString()) * (1 + i);
        ...
        private static bool ValidateRS(string value)
        {
            ...
            for (int i = 1; i < 9; i++) sum += int.Parse(value[i].ToString()) * (10 - i);
