---
type: req
title: "req-0001 — CPF — Validação"
description: "Validação sintática do dígito verificador do CPF para uso por aplicações .NET que processem documentos fiscais brasileiros. Formatação (máscara) é tratada no req-0014."
resource: "./requirement/req-0001-cpf-validacao.md"
...
Disponibilizar validação sintática do dígito verificador do CPF (Cadastro de Pessoas Físicas), para uso por aplicações .NET que processem documentos fiscais brasileiros. A formatação (máscara) `000.000.000-00` é tratada no requisito independente `req-0014`.

## Escopo

- **In scope:** validação do CPF (11 dígitos, com cálculo dos dígitos verificadores), obtenção do estado emissor do CPF, normalização de entrada via `OnlyNumbers` (aceita CPF com pontuação).
- **Out of scope:** formatação/máscara (coberta em `req-0014`), consulta a bases externas (Sefaz, Receita Federal), validação de situação cadastral (CPF regular/irregular), autenticação de usuários, persistência.
...
- `tec-req-0001-cpf-validacao.md` — especificação técnica
...
- `req-0014-cpf-mascara.md` — máscara do CPF (requisito base de formatação)
...
- **Gatilho:** Chamada ao método `IsCpfValid()` em uma string.
- **Resultado esperado:** `true` ou `false` para validação.
...
| RF-003 | O sistema deve identificar o estado emissor do CPF a partir do 9º dígito                                         | Média      | RN-004                     |
| RF-004 | O sistema deve validar entradas nulas ou vazias retornando `false`                                               | Alta       | RN-003                     |
...
| RN-004 | O 9º dígito do CPF indica a UF de emissão do documento (ex.: 1 = DF/GO/MS/TO, 8 = SP).                                                                                                                                 | Legislação          | RF-003                 |
...
| Desempenho       | Alocação zero no hot path | O método IsValid não deve alocar strings intermediárias desnecessárias             |
| Segurança        | Proteção contra ReDoS     | Regex simples (OnlyNumbers), sem backtracking catastrófico                         |
| Segurança        | Dados sensíveis em logs   | Documentação alerta para não logar CPF bruto; mascarar antes de logar (ver req-0014) |
...
- Classe `CpfValidation` com métodos `IsValid()`, `GetIssuingState()`
- Classe `CpfRule` com pesos de cálculo dos dígitos
- Extension method `CpfExtension` com `IsCpfValid()`
- Todas em `Sirb.Validation.Documents.BR.Validation` e `Sirb.Validation.Extensions`
- Formatação (máscara) em `req-0014` (`PlaceCpfMask` / `PlaceMask`)
...
| 31/07/2026 | Rodrigo Araujo Barbosa | 1.1.0  | Remoção do conteúdo de máscara (formatação transferida para req-0014); escopo reduzido a validação pura; RF/RN renumerados; APF recalculado para 6 PF |
...
| 27/07/2026 | Qual o comportamento esperado para CPF vazio/nulo?      | Retornar `false` para IsValid.                                                                                      | Resolvido                   | Criação                        | Define critério de aceitação para entradas vazias.         |
...
| 27/07/2026 | O CPF aceita entrada com caracteres não numéricos?      | Sim. OnlyNumbers é chamado internamente antes da validação, normalizando a entrada.                                 | Resolvido                   | Criação                        | Define comportamento de entrada com pontuação.             |
...
| 31/07/2026 | Onde fica a formatação (máscara) do CPF após a revisão? | No requisito independente req-0014 (PlaceCpfMask/PlaceMask). Este requisito passa a descrever apenas validação.      | Resolvido                   | Implementação                  | Escopo reduzido; RF/RN de máscara removidos e renumerados. |
...
| 2   | Fluxos principais e alternativos      | Resolvido | RF-001 a RF-004, cenários Gherkin                                                   | —                                                            |
...
| 6   | Dados e privacidade                   | Resolvido | CPF é dado sensível (LGPD); não logar valores brutos; mascarar antes de logar (req-0014) | —                                                       |
| 7   | Validações e regras de negócio        | Resolvido | RN-001 a RN-004, algoritmo módulo 11, rejeição de dígitos repetidos                 | —                                                            |
