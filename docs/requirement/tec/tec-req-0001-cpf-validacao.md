---
type: tec-req
title: "tec-req-0001 — CPF — Validação (Especificação Técnica)"
description: "Especificação técnica da validação sintática do CPF, algoritmo módulo 11 e identificação de estado emissor. Formatação (máscara) é tratada no tec-req-0014."
resource: "./requirement/tec/tec-req-0001-cpf-validacao.md"
...
domain:
  ...
  title_pt: "CPF — Validação — Especificação Técnica"
  version: "1.1.0"
...
# tec-req-0001 — CPF — Validação (Especificação Técnica)
...
- **Título:** CPF — Validação — Especificação Técnica
- **Data de criação:** 27/07/2026
- **Última atualização:** 31/07/2026
- **Autor:** Rodrigo Araujo Barbosa
- **Versão:** 1.1.0
- **Status:** Aprovado
- **Requisito de negócio relacionado:** `req-0001-cpf-validacao.md` (v1.1.0)

## 1. Resumo do requisito de negócio

- **Requisito de origem:** `req-0001-cpf-validacao.md` (versão 1.1.0)
- **Síntese do "o quê":** Validar sintaticamente CPF (11 dígitos, módulo 11) e identificar estado emissor. Formatação (máscara) é coberta pelo `tec-req-0014`.
...
Extensions (API Pública)
  └── CpfExtension.cs               (IsCpfValid via extension method)
Validation (Orquestração)
  └── CpfValidation.cs               (IsValid, GetIssuingState)
...
| RF-003 | Identificar estado emissor pelo 9º dígito | Média | RN-004 |
...
| RN-004 | 9º dígito indica UF de emissão (vide tabela na seção 2.4) | Legislação | RF-003 |
...
```mermaid
flowchart TD
    A[Entrada: string CPF] --> B[OnlyNumbers]
    B --> C{Tamanho == 11?}
...
sequenceDiagram
    ...
    Val->>Val: OnlyNumbers() → "12345678909"
...
| ------- | ------- | --------- | ------ |
| CPF com pontos e traço | "123.456.789-09" | IsValid = true | OnlyNumbers normaliza |
...
| CPF com espaços | " 12345678909 " | IsValid = true? Depende se o fluxo de validação normaliza espaços | OnlyNumbers mantém apenas dígitos → "12345678909" → válido |
...
| Item | Referência |
| ---- | ---------- |
| Requisito de negócio | `req-0001-cpf-validacao.md` |
| Requisito de máscara (formatação) | `tec-req-0014-cpf-mascara.md` |
...
| 31/07/2026 | Rodrigo Araujo Barbosa | 1.1.0 | Remoção do conteúdo de máscara (formatação transferida para tec-req-0014); RF/RN renumerados; OnlyNumbers no fluxo |

### 8.2 Clarification Log

| Data | Pergunta | Resposta | Status | Origem | Impacto |
| ---- | -------- | -------- | ------ | ------ | ------- |
| 27/07/2026 | O OnlyNumbers é a normalização usada na validação? | Sim. O fluxo de validação normaliza a entrada via OnlyNumbers (regex `[^\d]`). | Resolvido | Criação | Define comportamento de normalização. |
...
| 31/07/2026 | Onde fica a formatação (máscara) após a revisão? | No tec-req-0014 (PlaceCpfMask/PlaceMask). Este tec-req descreve apenas validação e estado emissor. | Resolvido | Implementação | Escopo reduzido; RF/RN de máscara removidos. |
...
| 6 | Dados e privacidade | Resolvido | CPF sensível; mascarar antes de logar (req-0014) | — |
